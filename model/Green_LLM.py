...
def green_llm_optimization(params):
    ...
    model.setObjective(C1 + C2 + C3 + C4 + C5 + C_delay, GRB.MINIMIZE)
...
# Define processing delay (Equation 15)
    model.addConstrs((D_proc[i, k, t] == gp.quicksum(v[j, t] * (f[k]+h[k]) * lmbda[i, k, t] * x[i, j, k, t] for j in J) for i in I for k in K for t in T),name="proc_delay_def")
